...
- **`images/base/Dockerfile`** — builds from `mcr.microsoft.com/devcontainers/base:ubuntu-24.04`. Runs `apt-get upgrade` first, so the packages the upstream base already ships get security patches (nothing pins an apt version, so without it they stay frozen at whatever the upstream build had); then installs the tooling common to every image: Azure CLI, GitHub CLI, Node.js, pre-commit. Tools are installed from version-pinned URLs and verified at build time against the checksum the upstream project publishes for that version (checkov against the GitHub release API digest); Azure CLI and ble.sh, which publish no checksum, keep a hand-maintained `@sha256:` digest suffix.
