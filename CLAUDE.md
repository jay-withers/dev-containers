...
make smoke-test  # run each image's smoke tests against the locally built images
```
...
- **`images/base/Dockerfile`** — builds from `mcr.microsoft.com/devcontainers/base:ubuntu-24.04`. Runs `apt-get upgrade` first, so the packages the upstream base already ships get security patches (nothing pins an apt version, so without it they stay frozen at whatever the upstream build had); then installs the tooling common to every image: Azure CLI, GitHub CLI, Node.js, PowerShell, pre-commit, and the Docker CLI. Tools are installed from version-pinned URLs and verified at build time against the checksum the upstream project publishes for that version (checkov and pre-commit against the GitHub release API digest instead, since neither publishes a checksum file); Azure CLI, ble.sh, and the Docker CLI, which publish no checksum, keep a hand-maintained `@sha256:` digest suffix.
...
Specialised images switch to `USER root` to install, then back to `USER vscode`. To add a new image, create `images/<name>/Dockerfile` `FROM` the base, add `images/<name>/smoke-tests` listing the tools it adds, and add it to the `leaves` and `leaves-merge` matrices in `cd-publish.yml` and the build matrix in `ci-container-build.yml`. The smoke-test step itself needs no change — it reads the image's own list.

### Smoke tests

Each image declares the commands that prove its tooling works in **`images/<name>/smoke-tests`**: one shell command per line, `#` comments allowed. **`scripts/smoke-test.sh`** runs them inside the built image (a single `docker run` per image, `set -e`, output left visible since the printed versions are half the value when a wrong-architecture binary is the problem), and `make smoke-test` wraps it. `ci-container-build.yml` calls the same script, so CI and local runs can't drift.

The list lives beside the Dockerfile that installs the tools rather than in the workflow, so adding a tool means editing two files in one directory. Each image lists **only what it adds** — leaf images never re-test the base tooling they inherit — because each image is smoke-tested in the job that builds it, which is what makes a failure name the image that actually broke. Images and their lists are both discovered from `images/`, so a new image needs no change to the script or the workflow step. A missing `smoke-tests` file is an error rather than a silent pass. `ARCH=<arch>` additionally asserts the image reports that architecture before running anything, which is how a wrong-asset download gets caught; CI sets it from the build matrix.

### Docker access (Docker outside of Docker)

The base image ships the Docker **client** only — `docker`, plus the `buildx` and `compose` CLI plugins in `/usr/local/lib/docker/cli-plugins/`. There is no `dockerd` in any image: commands talk to the *host's* daemon through a socket the consuming repo bind-mounts, so containers started from a dev container are siblings on the host, not nested children. That needs no privileged mode.

Non-root access to that socket cannot be resolved at build time, because who owns the socket is a property of the host. **`images/base/docker-socket-setup.sh`** (installed as `/usr/local/bin/docker-socket-setup`) reconciles it at container start and handles both cases: a group-owned socket (native Linux) has its GID adopted by the image's `docker` group — or, if another group already holds that GID, that group is used instead — while a root-owned socket (what Docker Desktop mounts) is proxied by socat to a second, user-owned socket, with `DOCKER_HOST` pointed at it via `/etc/profile.d/10-docker-host.sh` that the image's `.bashrc` sources. It needs root, is idempotent, and is opt-in: consuming repos add `"postStartCommand": "sudo /usr/local/bin/docker-socket-setup"`. Without that hook the CLI still works as root and via `sudo`. Deliberately not an `ENTRYPOINT`: base has none, and adding one would change startup behaviour for every image built `FROM` it.

### Architecture handling

Every image is published as a multi-arch manifest covering `linux/amd64` and `linux/arm64`. The tool downloads are architecture-specific, so each `ARG` holds the URL of the **arm64** asset and the install step rewrites that token to match BuildKit's `TARGETARCH` before downloading. Upstream naming is inconsistent — Node and PowerShell use `x64`, kubectx uses `x86_64`, checkov uses `X86_64`, the Docker CLI and compose use `x86_64` paired with `aarch64` (the only tools that rename the arm64 side too), and everything else uses `amd64` — so those map the token explicitly via a `case`; the rest substitute `TARGETARCH` directly. Anything other than `amd64`/`arm64` fails the build loudly rather than silently downloading the wrong binary.
...
- **`.github/workflows/ci-container-build.yml`** — runs on PRs that change `images/**`. Builds the base image for `linux/amd64` and `linux/arm64`, each on a native runner of that architecture (`ubuntu-24.04` and `ubuntu-24.04-arm`), then builds the terraform and k8s images (`FROM` the matching per-arch base). Every image is smoke-tested via `scripts/smoke-test.sh` in the job that builds it, on both architectures — so the base job proves the base tooling before anything is built `FROM` it (it loads as well as pushes, to run the image it just built), and each leaf job covers only its own additions. Because the leaf jobs run on different runners than the base jobs, the base is pushed to GHCR under a throwaway `pr-<run_id>-<arch>` tag; no release tag is published.
...
- **`.github/workflows/cd-scan.yml`** — reusable (`workflow_call`, invoked as the last job of `cd-publish` against the version just published) and manual (`workflow_dispatch`, to scan any published tag). Scanning at publish time rather than on a schedule ties each report to an exact immutable tag instead of whatever `latest` pointed at when a timer fired, and covers mid-week releases; `cd-weekly` still guarantees a report at least weekly. Because a nested reusable workflow can only narrow the permissions its caller grants, `issues: write` has to be declared on the `publish` job in **both** `cd-tag.yml` and `cd-weekly.yml`, not just here. Runs `scripts/scan-images.sh`, which scans the published images with Trivy for both platforms (read from the registry — nothing is executed or emulated) and emits a markdown report. **Report-only: a finding never fails the job**, since the fix for a CVE is a package upgrade or a base refresh, not a red build; a scan that fails to *run* does fail, so an incomplete report can't read as "nothing found". The report goes to the run summary and to a tracked issue labelled `vulnerability-report`. The body holds the full report and is refreshed every run; the comment is what emails the owner (a body edit sends no notification), so it carries only the short digest the script writes to `DIGEST_FILE` — mail clients don't collapse `<details>`, and the full report would mail every CVE table weekly. The issue title carries the counts, since it is the notification's subject line (lookup is by label, so the title is free to change), and a run where nothing appeared or cleared skips the comment entirely (`CHANGED` in `META_FILE`) so an email always means something moved. The report is organised by what would fix each finding: apt findings, which the next rebuild clears, are the headline; findings vendored inside a downloaded tool release are grouped by the binary that carries them and kept out of the headline, since nothing in this repo patches them and only an upstream release will. Each finding is attributed to its owning artifact using Trivy's own result target / `PkgPath`, so no per-tool mapping is maintained here. Detail tables are global with an *Images* column rather than per-image, because every image is built `FROM` base and per-image tables would repeat and inflate base's findings; findings dedupe on (CVE, package, owner). `linux-libc-dev` findings are counted separately rather than in the headline, because kernel headers ship no kernel and aren't reachable in a container. Each report ends with a hidden `scan-state` marker that the next run reads back out of the issue body (`PREVIOUS_REPORT`) to report what changed since last time. Images are discovered from `images/`, so a new image needs no change here.
